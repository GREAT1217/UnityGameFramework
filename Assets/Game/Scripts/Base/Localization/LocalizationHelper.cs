    ...
    public class LocalizationHelper : LocalizationHelperBase
    {
        ...
        public override bool ParseData(ILocalizationManager localizationManager, string dictionaryString, object userData)
        {
            try
            {
                ...
                string language = localizationManager.Language.ToString();
                dictionaryLineString = dictionaryString.ReadLine(ref position);
                ...
                {
                    Log.Warning("Can not parse dictionary line string '{0}' which  language column '{1}' is invalid.", dictionaryLineString, language);
                    return false;
                }

                int languageColumnIndex = MinColumnCount - 1;
                if (splitedLine.Length > MinColumnCount)
                {
                    for (int i = languageColumnIndex; i < splitedLine.Length; i++)
                    {
                        if (splitedLine[i] == language)
                        {
                ...
                while ((dictionaryLineString = dictionaryString.ReadLine(ref position)) != null)
                {
                    ...
                    splitedLine = dictionaryLineString.Split(ColumnSplitSeparator, StringSplitOptions.None);

                    string dictionaryKey = splitedLine[1];
                    string dictionaryValue = splitedLine[languageColumnIndex];
